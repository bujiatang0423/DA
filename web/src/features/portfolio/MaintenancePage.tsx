...
export function MaintenancePage(): JSX.Element {
  ...
  const load = async (): Promise<void> => {
    ...
    try {
      const [response, quotesResponse] = await Promise.all([fetch(`/api/v1/portfolio/positions?portfolio_id=${encodeURIComponent(portfolioId)}&as_of_time=${encodeURIComponent(new Date(asOfTime).toISOString())}`), fetch(`/api/v1/holdings/maintenance/quotes?portfolio_id=${encodeURIComponent(portfolioId)}`)]);
      if (!response.ok) throw new Error(`读取持仓失败（${response.status}）`);
      const data = await response.json() as PortfolioPositions;
      const quotes = quotesResponse.ok ? await quotesResponse.json() as Record<string, string> : {};
      setCash(trimDecimalZeros(data.cash)); setEquity(trimDecimalZeros(data.equity)); setVersion(data.version);
      setPositions(data.items.map((item) => ({ ...item, batch_id: `${item.origin}:${item.security_id}`, security_name: item.security_name ?? "", latest_price: quotes[item.security_id] ? trimDecimalZeros(quotes[item.security_id]) : null, average_cost: trimDecimalZeros(item.average_cost), effective_stop: item.effective_stop ? trimDecimalZeros(item.effective_stop) : null, highest_close: item.highest_close ? trimDecimalZeros(item.highest_close) : null })));
  ...
  const save = async (): Promise<void> => {
    ...
    setLoading(true); setError(null);
    try {
      const payloadPositions = positions.map((position) => ({
      ...
      const response = await fetch("/api/v1/holdings/maintenance", { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ portfolio_id: portfolioId, as_of_time: new Date(asOfTime).toISOString(), cash, equity, positions: payloadPositions, expected_version: version, reason }) });
      if (!response.ok) { const detail = await response.json().catch(() => null) as { message?: string; details?: { errors?: Array<{ loc?: Array<string | number>; msg?: string }> } } | null; const fields = detail?.details?.errors?.map((item) => `${item.loc?.join(".") ?? "字段"}: ${item.msg ?? "校验失败"}`).join("；"); throw new Error(fields ? `保存持仓失败：${fields}` : detail?.message ? `保存持仓失败：${detail.message}` : `保存持仓失败（${response.status}）`); }
