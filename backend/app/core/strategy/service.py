...
class V212StrategyEngine:
    def evaluate(self,request:StrategyEvaluationRequest)->StrategyEvaluation:
        ...
        for s in request.securities:
            p=policy_score(s.policy_evidence); f=financial_score(s.financial_numeric_score,s.financial_text_score,s.financial_light); q=list(s.quality_codes); reasons=[]; factors=None; sizing=None
            if not s.hard_filter_passed: q.append("HARD_FILTER")
        ...
        for s,fac,size,reasons,q in raw:
            pct=percentile_rank(scores,fac.s) if fac else 0; fac=FactorScores(fac.p,fac.f,fac.r,fac.t,fac.v,fac.s,pct) if fac else None; cons=check_constraints(ConstraintInput(request.portfolio,market.max_exposure,s.ledger,s.industry,s.theme,size.notional if size else 0,size.order_risk if size else 0,False)); out.append(SecurityEvaluation(s.security_id,s.name,s.industry,s.theme,fac,market.state,s.hard_filter_passed,s.policy_sources_available,s.llm_factor_valid,s.financial_light,s.policy_direction,s.breakout_confirmed,s.pullback_confirmed,s.strengthened_confirmed,s.days_since_breakout,s.held,s.close,s.ma20,s.ma60,s.atr14,s.r_multiple,pct,s.red_light,s.hard_stop,s.market_reduction,s.book_exit,s.rank_exit,s.stop_raise_required,size,cons,tuple(q),tuple(reasons)))
        out.sort(key=lambda x:(-(x.factors.s if x.factors else 0),x.security_id)); return StrategyEvaluation(request.as_of.as_of_time,request.strategy.version,request.manifest_hash,market,tuple(out))
