...
def assemble_snapshot(*, as_of_time: datetime, scope: SnapshotScope,
    ...
    for record in records:
        ...
        if record.available_at.astimezone(timezone.utc) > as_of_utc or event.astimezone(timezone.utc) > as_of_utc:
            raise FutureDataError(f"future record: {record.record_id}")
        if not record.entity_id.startswith("MARKET:") and allowed and record.entity_id not in allowed:
            continue
        if scope.history_start is not None and event.astimezone(timezone.utc) < scope.history_start.astimezone(timezone.utc):
            continue
