...
class AuditedPortfolioWriter:
    ...
    def record_manual_fill(self, command: object, expected_version: int)->PortfolioSnapshot:
        event=PortfolioAuditEvent(command.portfolio_id, 'manual_fill', datetime.now(timezone.utc), expected_version, '用户录入真实成交', _payload_hash(command))
    ...
    def replace_positions_for_correction(self,snapshot: object,expected_version: int,reason: str)->PortfolioSnapshot:
        ...
        event=PortfolioAuditEvent(snapshot.portfolio_id, 'position_correction', datetime.now(timezone.utc), expected_version, reason.strip(), _payload_hash(snapshot))
        return self._store.append(event=event,payload=snapshot,expected_version=expected_version)

def _payload_hash(payload: object) -> str:
    return hashlib.sha256(json.dumps(asdict(payload), sort_keys=True, default=str, separators=(',', ':')).encode()).hexdigest()
