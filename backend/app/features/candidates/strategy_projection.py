...
def project_security(
    ...
    *,
    ...
    bucket = (
        CandidateBucket.EXCLUDED
        if value.quality_codes or not value.constraint.allowed
        else CandidateBucket.EXECUTABLE
        if value.sizing and value.sizing.quantity
        else CandidateBucket.WATCHLIST
    )
    state = (
        CandidateState.HELD
        if value.held
        else CandidateState.PENDING_EXECUTION
        if bucket is CandidateBucket.EXECUTABLE
        else CandidateState.SELECTED
    )
