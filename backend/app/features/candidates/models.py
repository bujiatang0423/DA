...
class CandidateRecommendationResult:
    ...
    def executable(self) -> tuple[CandidateItem, ...]:
        return tuple(i for i in self.items if i.bucket is CandidateBucket.EXECUTABLE)

    @property
    def watchlist(self) -> tuple[CandidateItem, ...]:
        return tuple(i for i in self.items if i.bucket is CandidateBucket.WATCHLIST)

    @property
    def excluded(self) -> tuple[CandidateItem, ...]:
        return tuple(i for i in self.items if i.bucket is CandidateBucket.EXCLUDED)
