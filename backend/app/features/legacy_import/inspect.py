...
def _snapshot_at(p: Path) -> datetime | None:
    try:
        return datetime.strptime(p.name[:15], "%Y-%m-%d_%H%M").replace(
...
def inspect_source(source_root: Path) -> LegacyInspectionReport:
    ...
    for e in index:
        ...
        p = actual.get(name)
        if p is None:
            tags.add(LegacyQualityTag.MISSING_ARCHIVE)
            continue
        t = set()
        digest = _sha256(p)
        if digest != e.get("sha256"):
            t.add(LegacyQualityTag.CHECKSUM_MISMATCH)
        snap = _snapshot_at(p)
        if snap:
            with p.open(encoding="utf-8-sig", newline="") as f:
                if any(
                    r.get("buy_date") and date.fromisoformat(r["buy_date"]) > snap.date()
                    for r in csv.DictReader(f)
                ):
                    t.add(LegacyQualityTag.BUY_DATE_AFTER_SNAPSHOT)
        files.append(LegacyFileInspection(p, digest, snap, tuple(sorted(t))))
        tags.update(t)
    for name, p in actual.items():
        if name not in indexed:
            files.append(
                LegacyFileInspection(
                    p, _sha256(p), _snapshot_at(p), (LegacyQualityTag.UNINDEXED_FILE,)
                )
            )
            tags.add(LegacyQualityTag.UNINDEXED_FILE)
