...
def main() -> int:
    ...
    with sessions.begin() as session:
        batch = LegacyImportService(Path(args.imports_root), SqlLegacyRepository(session)).import_source(
            source_root=Path(args.source_root), portfolio_id=args.portfolio_id, effective_at=effective_at)
    print(json.dumps({"batch_id": batch.batch_id, "effective_at": batch.effective_at.isoformat(),
                      "manifest_sha256": batch.manifest_sha256, "idempotent": batch.idempotent},
                     ensure_ascii=False))
    return 0
