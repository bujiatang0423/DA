...
_OFFICIAL_HOSTS = frozenset(("csrc.gov.cn", "gov.cn", "sse.com.cn", "szse.cn"))
...
class OfficialEvidenceStore:
    ...
    def import_document(self, document: OfficialEvidenceDocument) -> None:
        ...
        with self._sessions.begin() as session:
            ...
            if existing is None:
                session.add(
                    OfficialEvidenceRow(source_host=host, **asdict(document))
                )
...
def _record(document: OfficialEvidenceDocument) -> TemporalRecord:
    ...
    entity_id = document.security_id or "MARKET:POLICY"
    suffix = (
        f"{document.report_period.isoformat()}:{document.content_sha256[:16]}"
        if document.report_period
        else document.content_sha256
    )
    available_at = max(document.published_at, document.first_observed_at, document.reviewed_at)
...
def _validate_document(document: OfficialEvidenceDocument) -> str:
    ...
    if len(document.content_sha256) != 64 or any(
        character not in "0123456789abcdef" for character in document.content_sha256.lower()
    ):
...
def _is_available(document: OfficialEvidenceDocument, as_of_time: datetime) -> bool:
    return max(document.published_at, document.first_observed_at, document.reviewed_at) <= as_of_time
