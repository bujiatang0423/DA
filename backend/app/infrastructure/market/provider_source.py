...
class ProviderResearchSource:
    ...
    def fetch(self, *, as_of_time: datetime, scope: SnapshotScope) -> ResearchBatch:
        ...
        rows = tuple(
            b
            for sid in scope.security_ids
            for b in self.provider.daily_bars(sid, start, as_of_time.date())
        )
        digest = hashlib.sha256(
            json.dumps([b.__dict__ for b in rows], default=str, sort_keys=True).encode()
        ).hexdigest()
        records = tuple(
            TemporalRecord(
                f"{self.provider.provider_name}:{b.security_id}:{b.trade_date}",
                DataKind.DAILY_BAR_RAW,
                b.security_id,
                datetime.combine(b.trade_date, time(15), self._timezone),
                as_of_time,
                datetime.combine(b.trade_date, time(15, 30), self._timezone),
                digest,
                {
                    "open": str(b.open),
                    "high": str(b.high),
        ...
        return ResearchBatch(
            ...
            (
                LineageRef(
                    f"{self.provider.provider_name}-{digest[:16]}",
