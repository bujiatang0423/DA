...
from backend.app.core.market.pit_models import DataKind, PointInTimeSnapshot, SecurityObservation, SnapshotQuality, SnapshotScope, TemporalRecord
from backend.app.core.market.strategy_inputs import StrategyInputBuilder, atr14, moving_average, obv_slope, winsorized_percentile
...
def _snapshot(records: tuple[TemporalRecord, ...]) -> PointInTimeSnapshot:
    ...
    return PointInTimeSnapshot(datetime(2026, 1, 1, tzinfo=UTC), SnapshotScope(), DataGrade.PIT_VERIFIED,
                               market, tuple(SecurityObservation(k, tuple(v)) for k, v in grouped.items()),
                               SnapshotQuality(()), (), "a" * 64)


def test_builder_maps_policy_llm_and_marks_incomplete_financial_quality() -> None:
    bars = tuple(_r(f"bar{i}", DataKind.DAILY_BAR_RAW, "AAA", {"trade_date": f"2025-01-{i+1:02d}", "close": 10 + i, "high": 11 + i, "low": 9 + i, "volume": 100000, "amount": 60000000}) for i in range(20))
    rows = (_r("breadth", DataKind.REALTIME_QUOTE, "MARKET:INDEX", {"breadth": 0.6, "close": 10}),
            _r("master", DataKind.SECURITY_MASTER, "AAA", {"name": "Alpha", "listing_days": 200}),
            _r("policy", DataKind.POLICY_DOCUMENT, "AAA", {"strength": .8, "relevance": .7, "stage": "pilot", "evidence_confidence": .9, "data_completeness": .8}),
            _r("llm", DataKind.LLM_FACTOR, "AAA", {"factor": {}}), *bars)
    portfolio = PortfolioSnapshot("p", datetime(2026, 1, 1, tzinfo=UTC), 1, Decimal("0"), Decimal("100"), ())
    result = StrategyInputBuilder().build(snapshot=_snapshot(rows), portfolio=portfolio, strategy_version="v1.0")
